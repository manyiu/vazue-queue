...
| DynamoDB on-demand throughput | Scales with traffic; enroll hot keys spread via shard counters (`queue#shardN`); `serving` remains a hot read on status polls (mitigated by CloudFront on `standard`/`full`) |
...
**Enroll counter sharding:** Each enroll does one `ADD` on `queue#shard{S}` plus one consistent `BatchGetItem` across shards for position assignment (~8 keys by default). This spreads the former `queue#global` hot write across N counter items. Use **fresh events** after upgrading — do not mix legacy `queue#global` enrollments with shard-only allocation on the same event.
