#!/usr/bin/env bash
# In-region PROFILE=rc enroll burst on `standard` preset with enrollBuffer (async SQS enroll).
#
# Each virtual user performs one unique POST enroll (202 + pre-assigned request_id).
#
# Usage:
#   bash scripts/run-load-test-enroll-standard-inregion.sh
#   VUS=1000 bash scripts/run-load-test-enroll-standard-inregion.sh
#   POLL_AFTER_ENROLL=1 bash scripts/run-load-test-enroll-standard-inregion.sh
#   SKIP_DESTROY=1 bash scripts/run-load-test-enroll-standard-inregion.sh
set -euo pipefail
...
EVENT_ID="${EVENT_ID:-loadtest-enroll-standard}"
VUS="${VUS:-1000}"
PROJECT="${PROJECT:-vazue-loadtest-enroll-standard}"
ROLE_NAME="${ROLE_NAME:-vazue-loadtest-enroll-standard-codebuild}"
...
cleanup() {
  ...
  (cd "$ROOT/examples/load-test-enroll-standard" && unproxy npx cdk destroy "$STACK_NAME" --force)
  echo "==> CLEANUP done"
}
trap cleanup EXIT
...
BUCKET="vazue-loadtest-enroll-standard-${ACCOUNT}-$(date +%s)"
...
if [[ "${SKIP_DEPLOY:-}" != "1" ]]; then
  ...
  (cd "$ROOT/examples/load-test-enroll-standard" && unproxy npx cdk deploy "$STACK_NAME" --require-approval never)
fi
...
for i in $(seq 1 60); do
  code=$(curl -sS "${CURL_OPTS[@]}" -o /tmp/vazue-lt-enroll-std-wr.html -w '%{http_code}' "$WAITING_ROOM_URL/" || true)
  if [[ "$code" == "200" ]] && grep -qi 'html' /tmp/vazue-lt-enroll-std-wr.html; then
...
echo "==> warm buffered enroll path"
...
for i in $(seq 1 60); do
  code=$(curl -sS "${CURL_OPTS[@]}" -o /dev/null -w '%{http_code}' "$QUEUE_API_URL/v1/events/$EVENT_ID/status?request_id=$RID" || true)
  if [[ "$code" == "200" ]]; then
    echo "warmed request_id=$RID (worker caught up after ${i} polls)"
    break
  fi
  if [[ "$i" -eq 60 ]]; then
    echo "ERROR: status never became 200 for warm enroll (last HTTP $code)" >&2
    exit 1
  fi
  sleep 1
done
...
echo "==> CodeBuild role"
cat >/tmp/vazue-enroll-std-cb-trust.json <<'EOF'
{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"codebuild.amazonaws.com"},"Action":"sts:AssumeRole"}]}
EOF
unproxy aws iam create-role --role-name "$ROLE_NAME" --assume-role-policy-document file:///tmp/vazue-enroll-std-cb-trust.json >/dev/null
unproxy aws iam attach-role-policy --role-name "$ROLE_NAME" --policy-arn arn:aws:iam::aws:policy/CloudWatchLogsFullAccess
cat >/tmp/vazue-enroll-std-cb-s3.json <<EOF
...
EOF
unproxy aws iam put-role-policy --role-name "$ROLE_NAME" --policy-name s3access --policy-document file:///tmp/vazue-enroll-std-cb-s3.json
sleep 12
  ...
  build:
    commands:
      - |
        ...
        set +e
        QUEUE_API_URL="$QUEUE_API_URL" EVENT_ID="$EVENT_ID" PROFILE=rc VUS="$VUS" \\
          PRESET_NAME=standard ENROLL_BUFFER=1 POLL_AFTER_ENROLL="${POLL_AFTER_ENROLL:-}" \\
...
env_vars = [
  ...
  {"name": "PRESET_NAME", "value": "standard", "type": "PLAINTEXT"},
  {"name": "ENROLL_BUFFER", "value": "1", "type": "PLAINTEXT"},
]
if poll:
  env_vars.append({"name": "POLL_AFTER_ENROLL", "value": poll, "type": "PLAINTEXT"})
proj = {
  "name": os.environ["PROJECT"],
  "description": "Ephemeral in-region k6 buffered enroll burst (standard preset)",
...
open("/tmp/vazue-enroll-std-cb-project.json", "w").write(json.dumps(proj))
print("project ok")
PY

unproxy aws codebuild create-project --cli-input-json file:///tmp/vazue-enroll-std-cb-project.json >/dev/null
...
REPORT_JSON="$ROOT/docs/launch/load-test-enroll-standard-report-inregion.json"
unproxy aws s3 cp "s3://$BUCKET/load-test-report.json" "$REPORT_JSON"
unproxy aws s3 cp "s3://$BUCKET/k6.exit" /tmp/vazue-enroll-std-k6.exit || true
K6_EXIT=$(cat /tmp/vazue-enroll-std-k6.exit 2>/dev/null || echo 1)

DATE=$(date +%Y-%m-%d)
OUT_JSON="$ROOT/docs/launch/load-test-enroll-standard-${DATE}.json"
OUT_MD="$ROOT/docs/launch/load-test-enroll-standard-${DATE}.md"
cp "$REPORT_JSON" "$OUT_JSON"

python3 - "$OUT_MD" "$OUT_JSON" "$VUS" "$QUEUE_API_URL" "$WAITING_ROOM_URL" "$EVENT_ID" "$REGION" "$ACCOUNT" "$K6_EXIT" "$POLL_AFTER_ENROLL" <<'PY'
import json, sys
from datetime import datetime, timezone

out_md, out_json, vus, api, cf, event_id, region, account, k6_exit, poll_after = sys.argv[1:11]
...
sync_ref = "[`load-test-enroll-2026-08-29.md`](./load-test-enroll-2026-08-29.md)"

md = f"""# Load test enroll burst (`standard`, buffered) — {now.split()[0]}

In-region `PROFILE=rc` run at **{vus}** unique concurrent enrolls on the **`standard`** preset with **`enrollBuffer: true`**. Each virtual user performs one `POST …/enroll` (expects **202** + pre-assigned `request_id`; worker writes visitor asynchronously).
...
- **{vus} unique buffered enrolls** fired concurrently — production-recommended flash-traffic path (`standard` + SQS buffer).
- Status polling after enroll: **{poll_note}**.
- Compare with synchronous `minimal` enroll burst {sync_ref} (p95 ~1.4s) — buffered enroll should show much lower POST latency.
